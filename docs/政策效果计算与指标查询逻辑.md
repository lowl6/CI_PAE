...
> 一句话：我们看政策发布前后的指标变化，算出一个 0-1 的强度分，再转成 0-100 的效果指数给用户看。
...
### 存在哪里
- 表：`rel_policy_county`（记录"哪个县用了哪条政策"）
- 字段：`strength`（小数，0.2 到 0.95）
- 什么时候算：数据库初始化时自动跑脚本（`add_policy_enhancements.sql`）

### 算法逻辑：看前后变化

对比"政策发布前 2 年"和"发布后 1-3 年"的指标平均值，算出 4 个维度的加权分：

```
强度 = 经济 40% + 农业 30% + 基础设施 20% + 社会保障 10%
```
